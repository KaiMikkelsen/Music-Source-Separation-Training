...
def objective(trial: Trial, args: argparse.Namespace) -> float:
    ...
    #freqs_per_bands = trial.suggest_categorical("freqs_per_bands", [4, 8, 16, 32])
    mask_estimator_depth = trial.suggest_categorical("mask_estimator_depth", [2, 4, 6])
    ...
    augmentation_loudness_min = trial.suggest_loguniform("augmentation_loudness_min", 0.5, 1.0)
    augmentation_loudness_max = trial.suggest_loguniform("augmentation_loudness_max", 1.0, 2.0)
    #augmentation_mix = trial.suggest_loguniform("augmentation_mix", 0.1, 1.0)
    ...
    # **Inference settings**:
    dim_t = trial.suggest_categorical("dim_t", [16, 32, 64, 128])
    num_overlap = trial.suggest_categorical("num_overlap", [4, 8, 16])
    ...
    config.model.dim_head = dim_head
    config.model.heads = heads
    #config.model.freqs_per_bands = freqs_per_bands
    config.model.mask_estimator_depth = mask_estimator_depth
