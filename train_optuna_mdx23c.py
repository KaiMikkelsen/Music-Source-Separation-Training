...
def objective(trial: Trial, args: argparse.Namespace) -> float:
   ...
   # Model #
    ...
    growth = trial.suggest_int("growth", 64, 256, step=32)
...
if __name__ == "__main__":
    ...
    db_path = os.path.join(db_folder, f"mdx23c_optimization_{datetime.now().strftime('%Y-%m-%d')}_{unique_id}.sqlite3")
    ...
    study = optuna.create_study(
        ...
        study_name=f"mdx23c_optimization_{datetime.now().strftime('%Y-%m-%d')}_{unique_id}"
    )
